...
The values are returned with the `key` as well as the `doc_count` which specifies the number of documents in each bucket. By default, the buckets are sorted in descending order of `doc-count`.
...
Circonus features that rely on an accurate document count illustrate the importance of using the `_doc_count` field.
...
The `shard_size` property tells Circonus how many documents (at most) to collect from each shard.
...
The `significant_text` aggregation is similar to the `significant_terms` aggregation, but it's for raw text fields. Significant text measures the change in popularity between the foreground and background sets using statistical analysis. For example, it might suggest Tesla when you look for its stock acronym TSLA.
...
With `histogram` aggregations, you can visualize the distributions of values in a given range of documents very easily.
...
`distance_type` | Specify how Circonus calculates the distance. The default is `sloppy_arc` (faster but less accurate), but can also be set to `arc` (slower but most accurate) or `plane` (fastest but least accurate). Because of high error margins, use `plane` only for small geographic areas. | No
...
You can visualize the aggregated response on a map using Circonus Dashboards.

The more accurate you want the aggregation to be, the more resources Circonus consumes because of the number of buckets that the aggregation has to calculate. By default, Circonus does not generate more than 10,000 buckets. You can change this behavior by using the `size` attribute, but keep in mind that the performance might suffer for very wide queries consisting of thousands of buckets.
...
You can then use Circonus visualizations to represent this data with a network graph.
