...
1. Download [latest release](https://github.com/circonus-labs/circonus-logwatch/releases) from repository (or [build manually](#manual-build))
...
Create one config (JSON, YAML, or TOML) in `--log-conf-dir` for each distinct log. [See examples](https://github.com/circonus-labs/circonus-logwatch/tree/master/etc/log.d) in the repository.
...
1. Clone repo (outside of `GOPATH`) `git clone https://github.com/circonus-labs/circonus-logwatch && cd circonus-logwatch`
